...
class PayloadClient {
 public:
  ...
  arrow::Result<payload::manager::v1::AcquireRingSlotResponse> AcquireRingSlot(const std::string& ring_id) const;

  /// CommitRingSlot — publish the slot. Pass back the slot_idx,
  /// generation, and size_bytes from the matching AcquireRingSlot.
  arrow::Status CommitRingSlot(const std::string& ring_id, uint32_t slot_idx, uint64_t generation, uint64_t size_bytes) const;
  ...
  /// this capture.
  arrow::Result<payload::manager::v1::LeaseRingSlotResponse> LeaseRingSlot(const std::string& ring_id, uint32_t slot_idx, uint64_t generation) const;
