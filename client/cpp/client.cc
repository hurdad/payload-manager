...
arrow::Result<std::shared_ptr<arrow::MutableBuffer>> PayloadClient::OpenMutableBuffer(
  ...
  if (descriptor.has_gpu()) {
    ...
    return arrow::Status::NotImplemented(
        "Payload descriptor tier GPU is not available because this client was built without CUDA support "
        "(PAYLOAD_CLIENT_ARROW_CUDA=0)");
#endif
  }
...
arrow::Status PayloadClient::ValidateHasLocation(
  ...
  if (descriptor.has_gpu()) {
    ...
    return arrow::Status::Invalid(
        "payload descriptor requested GPU tier, but this client was built without CUDA support "
        "(PAYLOAD_CLIENT_ARROW_CUDA=0)");
#endif
  }
...
arrow::Status PayloadClient::ReleaseRingSlot(const std::string& lease_id) const {
