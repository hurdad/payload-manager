...
RamArrowStore::Allocate(const PayloadID& id, uint64_t size_bytes) {
...
*/
std::shared_ptr<arrow::Buffer>
RamArrowStore::Read(const PayloadID& id) {

  std::shared_lock lock(mutex_);

  auto it = buffers_.find(Key(id));
  if (it == buffers_.end())
    throw std::runtime_error("RAM payload not found");
...
*/
void RamArrowStore::Write(const PayloadID& id,
