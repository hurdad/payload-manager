...
SqliteRepository::SqliteRepository(std::shared_ptr<SqliteDB> db) : db_(std::move(db)) {
}
...
Result SqliteRepository::Translate(sqlite3* db, int rc) {
  if (rc == SQLITE_OK || rc == SQLITE_DONE || rc == SQLITE_ROW) return Result::Ok();
...
Result SqliteRepository::InsertPayload(Transaction& t, const model::PayloadRecord& r) {
  ...
  const char* sql = "INSERT INTO payload(id,tier,state,size_bytes,version) VALUES(?,?,?,?,?);";
...
std::optional<model::PayloadRecord> SqliteRepository::GetPayload(Transaction& t, const std::string& id) {
  ...
  const char* sql = "SELECT id,tier,state,size_bytes,version FROM payload WHERE id=?;";
...
Result SqliteRepository::UpdatePayload(Transaction& t, const model::PayloadRecord& r) {
  ...
  const char* sql = "UPDATE payload SET tier=?,state=?,size_bytes=?,version=? WHERE id=?;";
...
std::vector<model::LineageRecord> SqliteRepository::GetParents(Transaction& t, const std::string& id) {
  auto* db = TX(t).Handle();
...
std::vector<model::LineageRecord> SqliteRepository::GetChildren(Transaction& t, const std::string& id) {
  auto* db = TX(t).Handle();
...
std::optional<model::StreamRecord> SqliteRepository::GetStreamByName(Transaction& t, const std::string& stream_namespace, const std::string& name) {
  auto*       db = TX(t).Handle();
  const char* sql =
...
std::optional<model::StreamRecord> SqliteRepository::GetStreamById(Transaction& t, uint64_t stream_id) {
  auto*       db = TX(t).Handle();
  const char* sql =
      "SELECT stream_id,namespace,name,"
      "COALESCE(retention_max_entries,0),COALESCE(retention_max_age_sec,0),created_at "
...
Result SqliteRepository::DeleteStreamByName(Transaction& t, const std::string& stream_namespace, const std::string& name) {
  auto*       db  = TX(t).Handle();
...
Result SqliteRepository::AppendStreamEntries(Transaction& t, uint64_t stream_id, std::vector<model::StreamEntryRecord>& entries) {
  auto* db = TX(t).Handle();
  ...
  if (sqlite3_prepare_v2(db, ins_sql, -1, &ins_st, nullptr) != SQLITE_OK) return Result::Err(ErrorCode::InternalError, sqlite3_errmsg(db));
...
std::vector<model::StreamEntryRecord> SqliteRepository::ReadStreamEntries(Transaction& t, uint64_t stream_id, uint64_t start_offset,
...
Result SqliteRepository::TrimStreamEntriesToMaxCount(Transaction& t, uint64_t stream_id, uint64_t max_entries) {
  auto* db = TX(t).Handle();
  if (max_entries == 0) {
    return Result::Ok();
  }
...
Result SqliteRepository::DeleteStreamEntriesOlderThan(Transaction& t, uint64_t stream_id, uint64_t min_append_time_ms) {
  auto* db = TX(t).Handle();

  const char* sql = "DELETE FROM stream_entries WHERE stream_id=? AND append_time<?;";
...
Result SqliteRepository::CommitConsumerOffset(Transaction& t, const model::StreamConsumerOffsetRecord& record) {
  auto* db = TX(t).Handle();
...
std::optional<model::StreamConsumerOffsetRecord> SqliteRepository::GetConsumerOffset(Transaction& t, uint64_t stream_id,
