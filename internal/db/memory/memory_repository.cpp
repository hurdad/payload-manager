...
std::optional<model::StreamRecord> MemoryRepository::GetStreamByName(
...
MemoryRepository::GetStreamById(Transaction& t, uint64_t stream_id) {
    auto& s = TX(t).View();
...
Result MemoryRepository::DeleteStreamByName(Transaction& t, const std::string& stream_namespace,
...
Result MemoryRepository::AppendStreamEntries(Transaction& t, uint64_t stream_id,
                                             std::vector<model::StreamEntryRecord>& entries) {
    auto& s = TX(t).Mutable();
...
std::vector<model::StreamEntryRecord> MemoryRepository::ReadStreamEntries(
...
std::vector<model::StreamEntryRecord> MemoryRepository::ReadStreamEntriesRange(
...
std::optional<model::StreamConsumerOffsetRecord> MemoryRepository::GetConsumerOffset(
