...
#include "arrow/arrow_storage.pb.h" // generated from arrow_storageproto

inline arrow::Result<std::pair<std::shared_ptr<arrow::fs::FileSystem>, std::string>> ResolveFileSystem(
  ...
  auto to_key_value_metadata = [](const auto& proto_map) -> std::shared_ptr<const arrow::KeyValueMetadata> {
  ...
  switch (filesystem_options.options_case()) {
    ...
    case pb::arrow::storage::FileSystemOptions::kGcs: {
      ...
      if (proto_credentials.anonymous()) {
        ...
        base_options = arrow::fs::GcsOptions::FromServiceAccountCredentials(proto_credentials.json_credentials());
        ...
        base_options = arrow::fs::GcsOptions::FromAccessToken(proto_credentials.access_token(), expiration);
      }
      arrow::fs::GcsOptions options = base_options;
      if (!proto_credentials.target_service_account().empty()) {
        options = arrow::fs::GcsOptions::FromImpersonatedServiceAccount(base_options.credentials, proto_credentials.target_service_account());
      }
      options.endpoint_override       = proto_options.endpoint_override();
      options.scheme                  = proto_options.scheme();
    ...
    case pb::arrow::storage::FileSystemOptions::kHdfs: {
      ...
      options.connection_config.extra_conf  = to_std_map(proto_options.connection_config().extra_conf());
...
inline arrow::Result<std::pair<std::shared_ptr<arrow::fs::FileSystem>, std::string>> ResolveFileSystem(const std::string&                path,
                                                                                                       const pb::arrow::storage::Common& common) {
  return ResolveFileSystem(path, common.filesystem(), common.filesystem_options());
}

inline arrow::Result<arrow::Compression::type> ResolveCompression(const std::string& path, pb::arrow::storage::Compression compression) {
  switch (compression) {
