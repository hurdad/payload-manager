...
const char* TierName(payload::manager::v1::Tier t) {
  switch (t) {
    case payload::manager::v1::TIER_GPU:  return "gpu";
    case payload::manager::v1::TIER_RAM:  return "ram";
    case payload::manager::v1::TIER_DISK: return "disk";
    default:                              return "?";
  }
}
...
const char* StateName(payload::manager::v1::PayloadState s) {
  switch (s) {
    case payload::manager::v1::PAYLOAD_STATE_ALLOCATED: return "allocated";
    case payload::manager::v1::PAYLOAD_STATE_ACTIVE:    return "active";
    case payload::manager::v1::PAYLOAD_STATE_SPILLING:  return "spilling";
    case payload::manager::v1::PAYLOAD_STATE_DURABLE:   return "durable";
    case payload::manager::v1::PAYLOAD_STATE_EVICTING:  return "evicting";
    case payload::manager::v1::PAYLOAD_STATE_DELETING:  return "deleting";
    default:                                            return "?";
  }
}
...
int main(int argc, char** argv) {
  ...
  if (tier_arg == "ram")       req.set_tier_filter(payload::manager::v1::TIER_RAM);
  else if (tier_arg == "disk") req.set_tier_filter(payload::manager::v1::TIER_DISK);
  else if (tier_arg == "gpu")  req.set_tier_filter(payload::manager::v1::TIER_GPU);
  ...
  for (const auto& p : resp.payloads()) {
    ...
    const std::string ttl = p.expires_at_ms() > 0
        ? std::to_string(static_cast<int64_t>(p.expires_at_ms() - now_ms) / 1000)
        : "-";

    std::cout << std::left
              << std::setw(38) << uuid
              << std::setw(6)  << TierName(p.tier())
              << std::setw(12) << StateName(p.state())
