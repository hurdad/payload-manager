#pragma once
// Thin interface to the OTel SDK used by the integration test.
...
// The caller uses those IDs to build the W3C traceparent it injects into gRPC
// calls, so every server-side child span shares the same trace.
