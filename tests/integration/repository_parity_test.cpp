...
void VerifyRestartDurability(BackendFactory& backend, const std::string& id) {
  ...
  auto repo = backend.make_repository();
  {
    ...
    LineageRecord lineage{.parent_id = id, .child_id = id + "-child", .operation = "copy", .role = "parent", .parameters = "{}",
                          .created_at_ms = NowMs()};
...
BackendFactory MakeSqliteFactory() {
  ...
  auto make_repo = [db_path]() {
    ...
    db->Exec("CREATE TABLE IF NOT EXISTS payload (id TEXT PRIMARY KEY, tier INTEGER NOT NULL, state INTEGER NOT NULL, size_bytes INTEGER NOT NULL, version INTEGER NOT NULL, expires_at_ms INTEGER);");
    db->Exec("CREATE TABLE IF NOT EXISTS payload_metadata (id TEXT PRIMARY KEY, json TEXT NOT NULL, schema TEXT, updated_at_ms INTEGER NOT NULL, FOREIGN KEY(id) REFERENCES payload(id) ON DELETE CASCADE);");
    db->Exec("CREATE TABLE IF NOT EXISTS payload_lineage (parent_id TEXT NOT NULL, child_id TEXT NOT NULL, operation TEXT, role TEXT, parameters TEXT, created_at_ms INTEGER NOT NULL, FOREIGN KEY(parent_id) REFERENCES payload(id) ON DELETE CASCADE, FOREIGN KEY(child_id) REFERENCES payload(id) ON DELETE CASCADE);");
