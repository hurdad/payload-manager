...
static void BenchSnapshotCacheHit(int n_payloads) {
  ...
  for (int i = 0; i < n_payloads; ++i)
    ids.push_back(fix.MakeRamPayload(4096).payload_id());
  ...
  auto      result     = TimedRun(
      "ResolveSnapshot cache hit n=" + std::to_string(n_payloads),
      0,
      iterations,
      [&] { fix.manager->ResolveSnapshot(target_id); });
  ...
  std::cout << std::left
            ...
            << std::setw(14) << std::fixed << std::setprecision(3) << result.per_op_us()
            << "\n";
}
...
static void BenchSnapshotCacheMiss(int n_payloads) {
  ...
  auto      result     = TimedRun(
      "HydrateCaches n=" + std::to_string(n_payloads),
      0,
      iterations,
      [&] { fix.manager->HydrateCaches(); });
