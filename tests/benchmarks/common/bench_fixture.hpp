...
class CopyingMemoryBackend final : public payload::storage::StorageBackend {
 public:
  ...
  void Write(const payload::manager::v1::PayloadID& id, const std::shared_ptr<arrow::Buffer>& src, bool /*fsync*/) override {
    ...
    if (src->size() > 0)
      std::memcpy(copy->mutable_data(), src->data(), static_cast<size_t>(src->size()));
    std::unique_lock lock(mutex_);
...
struct BenchFixture {
  ...
  explicit BenchFixture() {
    ...
    manager = std::make_shared<payload::core::PayloadManager>(
        std::move(storage), lease, nullptr, nullptr, repo);
  }
...
struct BenchResult {
  ...
  double per_op_us() const { return total_ns / iterations / 1e3; }
...
inline void PrintHeader() {
  std::cout << std::left
            << std::setw(44) << "benchmark"
            << std::setw(10) << "size"
            << std::setw(8)  << "iters"
            << std::setw(14) << "per-op (µs)"
...
inline void PrintResult(const BenchResult& r) {
  ...
  std::string throughput = r.payload_bytes > 0
      ? (std::ostringstream{} << std::fixed << std::setprecision(1) << r.throughput_mbs()).str()
      : "-";

  std::cout << std::left
            << std::setw(44) << r.name
            << std::setw(10) << size_str(r.payload_bytes)
            << std::setw(8)  << r.iterations
            << std::setw(14) << std::fixed << std::setprecision(2) << r.per_op_us()
            << std::setw(16) << throughput
            << "\n";
}

// Run fn() for `iterations` calls, preceded by `warmup` uncounted calls.
inline BenchResult TimedRun(const std::string& name, size_t payload_bytes, int iterations,
                            const std::function<void()>& fn, int warmup = 3) {
...
inline int IterationsFor(size_t payload_bytes,
